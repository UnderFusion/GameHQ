...
{
    return QStringLiteral("hwnd=0x%1 recreated=%2 style_applied=%3 foreground 0x%4 -> 0x%5 (%6)")
...
// our ex-style.
bool OverlayPresenter::ensureNoActivate(void* hwnd, OverlayPresentReport& report)
{
    if (!hwnd)
        return false;
    ...
    if (hwnd == m_styledHandle && (exStyle & OverlayWin32::kExNoActivate)) {
        return false;
    }

    m_api->setExtendedStyle(hwnd, exStyle | OverlayWin32::kExNoActivate);
    // Frame-cache the style change without moving, resizing, reordering or
    // — above all — activating.
...
OverlayPresentReport OverlayPresenter::present(const QRect& geometry)
{
    ...
    // Style first: the window must be unactivatable BEFORE it can be shown,
    // moved or raised, not afterwards.
    ensureNoActivate(m_api->nativeHandle(), report);
