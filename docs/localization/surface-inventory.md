...
| `startup-launcher` | Application UI | `src/main.cpp`, `src/launcher/LauncherMain.cpp` | Load the Qt app locale before constructing visible surfaces; the Win32 launcher is explicitly `deferred follow-up` (`t2`) to independent `STRINGTABLE`/`LoadStringW` resources. | Portable-import failures use Qt catalogs; all five launcher dialogs later use the persisted/default locale with English fallback. |
| `installer` | Installer | `packaging/GameHQ.iss` | Explicitly `deferred follow-up` to `p5-1` through `p5-4`: add `[Languages]` and per-language `.isl`/`CustomMessages` without Qt coupling. | Clean install, upgrade, running-app, active-update, uninstall, and silent scenarios pass for every installer locale. |
...
| `playnite-ui` | Application UI | `integrations/playnite/src/GameHQ.Playnite/Settings/**`, `GameHQPlugin.cs`, `extension.yaml` | Explicitly `deferred follow-up` (`t1`) to Playnite-native dictionaries for XAML, view-model states, dialogs, validation, menu, diagnostics, and metadata. | Plugin surfaces follow Playnite's locale and complete English fallback without loading Qt catalogs. |
