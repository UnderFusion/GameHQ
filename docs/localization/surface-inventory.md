...
| `installer` | Installer | `packaging/GameHQ.iss`, `packaging/generated/InnoLanguages.iss`, `packaging/generated/InnoCustomMessages.iss`, `packaging/i18n/**` | The canonical registry generates sixteen pinned `[Languages]` mappings, while eleven GameHQ-owned messages use native `[CustomMessages]`; `p5-3` and `p5-4` own locale handoff and final acceptance. | Mapping, message structure, pinned-toolchain compilation, and focused failure-path regressions pass; the later full installer matrix covers every locale. |
| `release-notes-offline` | Release note | `assets/release-notes/versions/**`, `assets/release-notes/generated/**`, `src/app/ReleaseNotes.*` | Ship schema-identical locale assets with explicit fallback; localize section titles and items without translating versions or URLs. | Schema/parity checks and offline fallback render the intended locale. |
