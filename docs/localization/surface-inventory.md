...
| `startup-launcher` | Application UI | `src/main.cpp`, `src/launcher/LauncherMain.cpp` | Load language configuration, resolve the effective locale, then install the translator/QLocale before any user-visible model, menu, notification, dialog, or QML is constructed; give the Win32 launcher an independent string-resource/locale path. | Portable-import and launcher failures appear in the selected/default locale. |
| `installer` | Installer | `packaging/GameHQ.iss` | Add `[Languages]` and per-language `.isl`/`CustomMessages`; localize wizard, task, run, maintenance, and uninstall strings. | Clean install, upgrade, running-app, active-update, and uninstall scenarios pass for every installer locale. |
...
| `playnite-ui` | Application UI | `integrations/playnite/src/GameHQ.Playnite/Settings/**`, `GameHQPlugin.cs`, `extension.yaml` | Use Playnite localization dictionaries for XAML, view-model states, dialogs, validation, menu, and metadata; keep it separate from Qt catalogs. | Plugin settings, errors, menu, and connection states switch through Playnite's locale mechanism. |
| `updater-helper` | Log-only diagnostic | `src/updater/**`, transaction/recovery helpers | Keep detached-helper console/log protocol English and stable; only translate errors after the main app receives and displays them. | Helper output and exit semantics remain byte-stable; mapped app-facing errors are localized. |
| `packaging-ci` | Developer-only text | `packaging/*.ps1`, `.github/workflows/**`, tests | Keep command output, thrown validation errors, CI labels, and test fixtures English. | Extraction rejects these paths. |
