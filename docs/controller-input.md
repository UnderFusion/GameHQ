...
- **The right stick scrolls like a mouse wheel (vertical only).** `StickNav::verticalBits` maps the right stick's Y axis onto the `RStickUp`/`RStickDown` bits (`gamepad.stick_right_up`/`_down`) using the same `AxisConfig` and hysteresis rules as the left stick. Default-bound to the repeating desktop actions `desktop.scroll_up`/`desktop.scroll_down`, which emit `desktopScroll(direction)`; QML routes it to whichever surface is showing — gallery grid, Settings options page, assignment editor dialog, Help, full release notes — moving only the viewport, never the selection. Decoded by the Sony and XInput backends; WinMM deliberately does not decode it (which of Z/R/U/V the right stick lands on is not standardized across generic pads, and a wrong guess would turn a trigger into phantom scrolling). GameInput keeps its right-thumbstick direction flags routed nowhere while standard controls remain shadow-only, but `StandardControlMap` can map them onto the same ids at Stage C re-enablement.
- **One physical button, one canonical control, on every backend.** Each backend normalizes its own raw report, so the same DualSense button used to arrive under a different canonical id depending on which API delivered that edge: the Sony HID and XInput views published `gamepad.trigger_left` for L2 while the WinMM view published the unnamed `gamepad.button.0`, and `gamepad.button.0` simultaneously meant *left stick click* over XInput. A saved binding therefore matched only while one particular backend happened to be routing — the reported "after pressing some buttons the actions no longer match". Thumbstick clicks now use the canonical `gamepad.thumb_left`/`gamepad.thumb_right` (which GameInput's `StandardControlMap` already published) and triggers use `gamepad.trigger_left`/`_right` in every backend. A generic `gamepad.button.N` code is index-scoped, never device-scoped, so it may only be used for a control **no other backend can see** — after this correction the sole remaining one is the DualSense touchpad click over WinMM, which keeps its historic index so an existing binding to it survives. `tst_providercontrolmap` drives the three pure mapping tables (`WinMMDevice::mapDigitalButtons`, `XInputDevice::mapDigitalButtons`, `DualSenseDevice::decodeButtons`) and fails if any named position regresses to a generic index.
- **Because the right stick leaves focus behind, directions re-enter from the viewport.** A long scroll can push the focused control far off screen, and stepping from it would scroll the page straight back. `SettingsPage.focusInViewport()` tests whether the focused control still overlaps the visible band; when it does not, `SettingsView.enterFromViewport()` hands the step to `PadNav.viewportTarget()`, which picks the topmost on-screen focusable for Down and the bottommost for Up (nearest control outside the band when a tall text block leaves none inside). `focusOptions()` uses the same entry point, so entering the options panel from the category list also lands on what the user is looking at — on an unscrolled page that is still the first control. Left keeps its edge meaning and backs out to the categories.
...
**Where it bites.** `BindingResolver::reload()` validates every `binding_overrides` row it reads. A row that cannot form a valid pattern — an unknown chord version, the same control twice, a `press` carrying a hold duration, a value corrupted outside the app — is **skipped**, so the action falls back to its default instead of firing something nobody assigned. Each skip is logged (`Bindings: skipping stored override …`) and recorded through `InputDiagnostics::noteRejectedBinding()`, which surfaces it in the copied diagnostics under *Rejected binding rows*. The copied diagnostics also list the effective bindings in two sections: `[controller]` for the shared group table every pad inherits, and `[controller:<fingerprint>]` for the pad that last pressed something (`BindingRuntime::setActiveProfile`). Only the group rows used to be published, so a per-pad override was invisible in a bug report — the reporter and the developer were reading different binding tables without knowing it. Cleared slots (`unbound`, no trigger) are validated on the gesture half only, so the gesture-inheritance sentinels below survive untouched.
...
**Scope overlap.** Global is always live, so it overlaps everything, including when the runtime represents an absent contextual fallback as Global. Two *different* contextual scopes never overlap: once the primary scope binds an exact trigger, it owns that trigger across press, tap and hold gestures; the contextual fallback is consulted only when the primary has no binding for that trigger. Playback-over-Desktop and Playback-over-Overlay are therefore priority chains, not collisions. This is why D-pad Left can ship as both `playback.seek_back` and `desktop.navigate_left`, while Cross cannot pause a clip and then leak through to Desktop Confirm or Bulk Select.
...
See [product-spec.md §6](product-spec.md#6-controller-mapping-default). PS button is frequently intercepted (Steam, DS4Windows, Game Bar) → treat as optional; fallback overlay toggle: **Share double-tap** and `Ctrl+Shift+G`. PS carries a tap/hold pair like Share: **tap** toggles the overlay (fires on release — deliberately tap, not press, so the hold cannot also open the overlay on the way down), **2 s hold** fires `global.toggle_desktop` ("Show / Hide GameHQ Window", bindable in Settings → Input): `App::toggleDesktopWindow()` summons the desktop window with real OS focus via the overlay's `ForegroundAcquirer` AttachThreadInput bypass, remembering the window the foreground was taken from; a second hold while the GameHQ window is focused hides it and hands the foreground back. When the overlay is open during a summon it steps aside without restoring focus to the game (`OverlayManager::hideForDesktopHandoff()`), and its remembered game window becomes the return address instead.
...
## Binding editor controller pinning (cpo-c05)

While **controller-specific** editing is on, the controller the editor shows is **pinned**. That pad owns the editor until the user changes the selection; a second API speaking (a mirrored backend event), a backend takeover and the selected pad disconnecting all arrive through the same `BindingEditorModel::setControllerProfile` seam and are ignored for identity — they are diagnostics, never a selection change. Without the pin, the next provider event silently retargeted the editor and the following write landed on another controller (or the shared profile) — the 2026-09-08 settings-switch bug.

The pinned fingerprint is also the write target: captures, clears and resets stamp `selectedProfile()` when they persist, so an edit always lands on the selected controller. Re-selection is explicit only: switching controller-specific editing off restores following (the next control activity moves the editor), and switching it on again pins the controller displayed at that moment. Runtime routing/correlation and backend arbitration are untouched: presses keep flowing under the canonical logical profile (conservative provider splitting, `cpo-c03c`), and no cross-provider identity inference is performed.

## Backend arbitration & mirror suppression (0.7.4)

One physical pad often reaches GameHQ through two APIs at once: Windows exposes every XInput pad through the legacy WinMM joystick surface, and remappers (DSX, reWASD) feed Raw Input, XInput and WinMM simultaneously. `ControllerArbitration` (`src/input/ControllerArbitration.h`) keeps one **active backend** and treats any cross-backend event within `BackendDuplicateWindowMs` (100 ms) of the active backend's last control as a suspect mirror. Two defenses (0.7.4, pinned by `tst_controllerarbitration` including a production-sequence regression that walks the exact `onControlPressed` → `resolvePendingCandidate` branch order):
...
`WinMMDevice` deliberately does **not** filter out the WinMM view of XInput pads (an interim filter written during 0.7.4 development was removed before release): VID:PID identifies a *model*, not a physical endpoint, so model-level suppression could hide a legacy-only pad that shares its VID:PID with an XInput one — and the arbitration drop above already neutralizes the mirrored stream. Endpoint-strong correlation (PnP/container id) is the only acceptable basis for a future filter.
...
## GameInput focus policy (0.7.4; scoped exclusive-foreground since cpo-o06c)

GameHQ normally runs *behind* the focused game, and GameInput's default focus policy delivers input — including the system Guide/Share buttons — only to the focused process. Every GameInput session therefore starts in the **background** policy, applied by `GameInputWrapper::start()` after `initialize()` and **before** any callback registration (`SetFocusPolicy(EnableBackgroundInput | EnableBackgroundGuideButton | EnableBackgroundShareButton)` — all three flags, because `EnableBackgroundInput` does not cover the system buttons). Order is contract-tested in `tst_gameinputwrapper`.

Since cpo-o06c there is a second, deliberately narrow state. While the in-game overlay holds the **verified interactive foreground** (the cpo-o06b condition: the overlay owns the foreground AND the game is alive, visible and un-minimized), GameHQ requests `ExclusiveForegroundInput | EnableBackgroundGuideButton | EnableBackgroundShareButton`. Exclusive-foreground input *replaces* background input for standard input — asking for both would contradict itself — and the two background system-button flags stay, because GameHQ's own Guide/Share handling (hold-PS desktop summon, Share capture) must behave exactly as before. `GameInputExclusiveForegroundGuideButton` and `...ShareButton` are deliberately **not** requested: taking the system buttons from the game as well is broader than this experiment needs.

The policy has exactly **one owner**: `GameInputFocusController`. Interactive surfaces only *request* a transition (`GameInputFocusRequestSink`); the controller decides the flags through the single `IGameInputApi::applyFocusPolicy()` seam, records the transition and its reason, and is the only thing that may leave the exclusive state. It requests the exclusive policy only after the verified interactive state, never when the foreground request was denied or cancelled, and releases it on **every** exit path — overlay close, desktop handoff, another application taking the foreground (including a polled fallback when no foreground event arrives), the game going away, GameInput being switched off, and runtime shutdown (released while the runtime is still alive to receive it). A fresh session always starts from the background policy. A **normal exit** releases the policy on the runtime-shutdown path while the runtime is still alive, and GameHQ sets no machine-level state anywhere — no virtual device, no cloaking configuration, nothing inside a game — so an abruptly killed process leaves only its own per-process GameInput state behind. That last sentence is an expectation from process lifetime, not a measured result; `docs/overlay.md` ("Crash, exit and recovery") says so in the same words and the crash experiment is listed with the physical acceptance work.

**What this does not do — do not read the exclusive state as input isolation.** GameInput's focus policy is best-effort and GameInput-scoped: it binds other *GameInput clients*, while games reading XInput, DirectInput or Raw Input directly are unaffected. Nothing inside GameHQ's process can observe whether another process actually stopped receiving input, so every open classifies itself from the facts it *does* have — the policy that ended up in force, plus the evidence recorded outside the app — and states its own position in the record: `isolation=scoped evidence=external-gameinput-clients real-game=unconfirmed`, `isolation=unavailable evidence=none real-game=unconfirmed`, or `isolation=unclassified (…)` when the open recorded none. A `full-native` claim needs a recorded physical result for that game, pad and configuration, and no API call in this process can produce one (`src/gameinput/IsolationCapability.{h,cpp}`, pinned exhaustively by `tests/tst_isolationcapability.cpp`). Which cases that covers, and which paths are explicitly *not measured*, is `docs/overlay.md` ("Controller isolation: what is verified, and what is not"). The exclusive state is not to be read as universal isolation.

**The external measurement (cpo-o06d, 2026-09-22).** That question is now answered from *outside* GameHQ. `tools/input-receiver/` builds a developer-only console observer (`gamehq_input_receiver.exe`, defined inside the test build, never part of any payload) that watches the pad from its own process through GameInput, XInput and a Raw Input `RIDEV_INPUTSINK` sink, and records phase by phase what actually arrived. Its GameInput path deliberately asks the runtime for background ordinary input, so a gap during the exclusive phase cannot be explained by the receiver having lost the foreground, and every activity/presence/phase line carries the receiver's own foreground state plus the foreground HWND, pid and title. With a test-owned window holding the foreground and the **shipped** policy path doing the asking (`GameInputFocusController` → `ProductionGameInputApi`), two independent runs on a DualSense (054c:0ce6, USB) measured `208.90/s → 0.00/s → 248.94/s` and `208.73/s → 0.00/s → 248.88/s`: another process's GameInput delivery really stopped while the exclusive policy was in force and really resumed after release. XInput and Raw Input were `not-measurable` in the same runs — the pad is not an XInput device and an idle pad produces no Raw Input reports for the registered collections — and that is reported as exactly that, never as isolation. See `tools/input-receiver/README.md` for the receipt format, the operational limits and the manual real-game procedure.

### Releasing the exclusive policy: the neutral handoff (`cpo-o06e`)

Handing the pad back is a transition in its own right, and the order is the point: the policy is released **after** the pad is neutral, never before — releasing first would expose the still-held state to the game during the wait. The close enters `ReleasingInput`, the overlay input path quiesces (no navigation repeat, no pending gesture able to fire), the wait polls at 10 ms inside a **400 ms** bound, and only then does the policy go back, followed by the foreground and the hide. `ModernInput::NeutralHandoffRunner` owns that order; `OverlayManager` owns the close, so each exit path releases exactly once.

**One definition of neutral** (`src/gameinput/NeutralHandoff.h`, `src/input/HeldControlTracker.h`): no digital control held, as published by the active backend. Thresholds and deadzones stay where they already are — GameInput's own trigger threshold and thumbstick deadzone, StickNav's per-backend `AxisConfig` — so the wait cannot disagree with the interpretation that routed the press. The tracking is fed by the raw press/release edges, before any routing, so a control that fired no action still counts; a device that disappears takes its held controls with it.

**Receipts** (log, diagnostics timeline and the overlay close line share one formatter): `neutral-handoff=waiting (…)`, `=passed duration_ms=… polls=…`, `=timeout duration_ms=… polls=… held="…"`, `=released-elsewhere …` (another path restored the policy first — the foreground probe, or the runtime stopping), `=not-engaged (…)` (the desktop handoff, a game window that is not a hand-back target, no policy in force, or no source to observe with).

A timeout is recorded as a timeout: the close completes, the policy and the foreground are restored, and nothing is synthesized into the game. Real-game confirmation that no action leaks stays with the physical acceptance work; what is automated here is the state machine, its order and the engine's raw-edge bookkeeping.
...
**Correlation.** `PhysicalControllerRegistry` matches an observation against the live controllers on ranked evidence, strongest first: app-local device ID, then endpoint, then container, then device root. Exact per-device evidence always wins — a container or a root can legitimately hold several endpoints, so ambiguous topology evidence may never preempt an exact match. Every rank demands exactly **one** candidate that contradicts nothing stronger, and container/root candidates additionally require that the candidate has no live attachment from the same provider (a provider observes each physical device separately, so a same-provider match is the identical twin, not the same pad). Ambiguity deliberately creates a new identity instead of guessing. Because every rank is unique-or-nothing, the result never depends on `QHash` iteration order. The deterministic logical ID hashes the *same* ranked evidence, and a new identity that would collide with an existing one is disambiguated on the provider endpoint — two pads behind one receiver root can never collapse onto one ID. When a strong (GameInput) observation merges into a controller a legacy provider created first, the logical ID is upgraded to the deterministic strong hash so persisted per-controller state (binding profiles, extra-button layouts) survives provider arrival order.

**What legacy providers can and cannot correlate on.** VID:PID identifies a *model*, never a physical endpoint, so it is not a correlation key: merging on it would collapse two identical pads. Today only GameInput reports container and root evidence (`IGameInputDevice` container ID, root ID); Sony Raw Input, XInput and WinMM report an `endpointId` in a provider-private namespace (`hid.endpoint.<hash>` from the Raw Input device path; `winmm.slot<N>`), which can never compare equal across providers. **Cross-provider correlation therefore does not happen in production** — each backend's view of one pad stays a separate logical controller, which is the safe behaviour, not the desired one. Closing that gap needs a genuine shared OS identity, not a heuristic: the PnP container ID resolved from the HID device path (`CM_Locate_DevNodeW` + `DEVPKEY_Device_ContainerId`) is the same value GameInput already reports and is derivable by Sony Raw Input and by XInput whenever its Raw Input `IG_` correlation is unambiguous. WinMM cannot reach it — `joyGetDevCaps` exposes no interface path — so WinMM stays conservative.

**Cross-provider identity: the current production decision (0.7.35).** The Container ID key is deliberately **not wired into production**. Only GameInput emits container/root evidence; Sony Raw Input, XInput and WinMM stay on provider-private endpoints, so each backend's view of one pad remains a separate logical controller — conservative splitting is the shipped behaviour, not a regression. The OS container seam (`src/input/PnpDeviceApi.{h,cpp}`, `CM_Locate_DevNodeW` + `DEVPKEY_Device_ContainerId`) and its probe (`tools/spikes/pnp_container_probe.exe`) exist and are tested, but no provider consumes them: wiring waits for a real DualSense to prove the OS bytes-id container equals GameInput's `containerId` for the *same* physical pad, and for a migration/alias design so stored profiles keep resolving. Until both land, the key is treated as unavailable and refused; VID:PID, model name, visible-device count and XInput slot are never fallbacks.

**Share/Guide dedup (press-cycle ownership, 0.7.5).** Legacy `Capture`/`Guide` edges route through `ProviderIntegration::routeLegacySystemEdge` (fail-open if the attachment was never observed); GameInput system buttons route through the same shared router. On a merged controller the router models one physical press as one *cycle*: the first accepted press owns the control for that cycle; a mirrored press from another correlated provider joins the cycle as a participant but stays `duplicate` — it never transfers ownership and never emits a second action; a release from **any** participant closes the cycle exactly once; a release with no open cycle is explicitly ignored, so a late mirror release can never become a spurious action. Ownership only moves when the owning provider detached mid-hold — the stuck state is closed with a safe release and the working source opens a fresh cycle. (0.7.4 instead transferred a held control to the preferred mirror, which rejected the owner's release and left Share/PS logically held forever — every later press was swallowed as a duplicate.) Exactly one edge per physical press, proven by `tst_capabilityeventrouter`, `tst_providerintegration` and `tst_gameinputrouter`, including 20-cycle repeated Share/PS handoff regressions. If a legacy provider is connected but **not** correlated onto a GameInput controller (two identical pads, or a remapper reporting a different identity), GameInput Share/Guide edges are withheld (`shadowedSystemEdgeCount`) because dedup cannot be guaranteed; a release of an already-held control always passes so nothing sticks. When GameInput turns Off, shuts down or fails its session it detaches from the shared registry so the legacy path immediately owns Share/Guide again. GameInput edges execute bindings under the controller's logical-identity profile, so "this controller" assignments fire regardless of the delivering provider.

**Standard controls** remain shadow-only in GameInput (Stage C): standard-control dedup across providers is not yet proven end to end. The complete v3 flag map (`src/gameinput/StandardControlMap.h`, incl. L3/R3, C/Z, paddles; right-stick directions deliberately unmapped) is defined and tested for that re-enablement. Extra buttons whose device-reported label names a standard control are blanked from `Extra Button N` enumeration (`ExtraButtonCatalog::isStandardControlLabel`) so one physical button can't carry two identities.

## Selective Raw HID fallback (t26, 0.7.3)

For a dedicated button no backend exposes (some GameSir/8BitDo extras), the Raw Input backend runs a production fallback on *ignored* gamepad-class HID handles: `WM_INPUT` payload → `RawInputApi::buttonUsages` (RIDI_PREPARSEDDATA + `HidP_GetUsages`, `hid.dll`) → usage press/release transitions → `SelectiveRawHidFallback` (bound-control filter) → canonical `gamepad.raw.<device-hash>.usage.<page>.<usage>` control → `ProviderIntegration::routeRawHidEdge` (correlates onto the pad's logical controller by `vvvv:pppp`) → binding runtime. The idle guarantee is preserved: with no bound raw-HID control and no probe open the ignored path costs two branches (zero allocation, pinned by `tst_rawinputflood`); an unbound device with bindings elsewhere settles ineligibility once per handle. A device that disappears mid-press gets a synthesized release. Binding capture for these controls currently happens while the 3-second probe window is open.

## Layout reconfirmation (extra buttons)

When a controller's extra-button layout signature changes (firmware update, mode switch), old bindings stop routing and Settings → Input shows **Controller button layout changed** with two actions: **Review buttons** (runs the 3-second probe) and **Confirm current layout** (`InputEngine::confirmModernControllerLayout` → `GameInputRouter::confirmLayouts` → `ExtraButtonCatalog::confirm`). Confirmation clears `needs_reconfirmation` and extra buttons resume with the new layout — there is no permanent silent state.
...
- **Dedicated hook thread**: Windows delivers every `WH_MOUSE_LL` callback synchronously to the installing thread and the system pointer waits on it, so the hook lives on its own worker thread that does nothing but sleep in `GetMessage()`. Normal thread priority — the thread is idle whenever the hook is. `start()`/`stop()` are idempotent (`tst_mousehooklazy`, including a 25× rapid-cycle case).
- **Generation guard**: every hook event is stamped with the hook lifetime (`MouseHookDevice::generation()`, bumped on every start AND stop). A press queued cross-thread can arrive after the hook stopped — or stopped and restarted, which an `isRunning()` check would wrongly accept — and its release was never captured; InputEngine drops any press whose generation is not current (`tst_mousehooklazy::staleQueuedPressAcrossRestartIsRejected`). Releases process regardless of generation: releasing an unpressed control is a safe no-op, dropping a real one risks a stuck control.

Diagnostics for the remaining suspects ship alongside: `PerfTrace::reportSlow` logs any XInput poll/rescan or WinMM poll pass above 2 ms (rate-limited, `Perf:` prefix), and `EventLoopStallMonitor` logs GUI event-loop stalls ≥ 50 ms the same way, so one user log shows whether hitches, slow backend calls and main-thread stalls line up. The WinMM discovery sweep itself no longer runs on the GUI thread at all (measured at 161 ms; see `WinMMDevice::scanSlots`) — it executes on a short-lived worker with an in-flight guard, and only the result is marshalled back to the owning thread.
