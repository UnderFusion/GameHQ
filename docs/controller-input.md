...
**What this does not do — do not read the exclusive state as input isolation.** GameInput's focus policy is best-effort and GameInput-scoped: it binds other *GameInput clients*, while games reading XInput, DirectInput or Raw Input directly are unaffected. Nothing inside GameHQ's process can observe whether another process actually stopped receiving input, so every record of the transition says `isolation=not measured in-process`; the exclusive state is described as "effect not verified in-process" in the diagnostics export. The external receiver measurement is the separate cpo-o06d leaf, reported in the next paragraph, and the shipped Exclusive Controller Mode design (`exclusive-controller-mode.md`) is untouched by this.
